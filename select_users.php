...
$labid = required_param('labid', PARAM_INT);
...
$PAGE->set_url('/mod/ejsappbooking/select_users.php', array('labid' => $labid, 'bookingid' => $bookingid, 'id' => $id, 'courseid' => $courseid, 'contextid' => $context->id));
...
$rolenamesurl = new moodle_url("$CFG->wwwroot/mod/ejsappbooking/select_users.php?labid=$labid&bookingid=$bookingid&id=$id&courseid=$courseid&contextid=$contextmodid&sifirst=&silast=");
...
$baseurl = new moodle_url('/mod/ejsappbooking/select_users.php', array(
      'labid' => $labid, 
      'bookingid' => $bookingid, 
      'id' => $id, 
      'courseid' => $courseid, 
...
if ($mycourses = enrol_get_my_courses()) {
  $courselist = array();
	$popupurl = new moodle_url('/mod/ejsappbooking/show_participants.php?labid='.$labid.'&bookingid='.$bookingid.'&id='.$id.'&courseid='.$courseid.'&contextid='.$contextmodid.'&roleid='.$roleid.'&sifirst=&silast=');
...
if ($bulkoperations) {
  echo "<form action=\"send_messages.php?courseid=$courseid&id=$id&labid=$labid&bookingid=$bookingid\" method=\"post\" id=\"participantsform\">" . '<div>';
