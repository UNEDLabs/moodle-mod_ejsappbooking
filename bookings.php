...
$date = DateTime::createFromFormat("Y-m-d", $selectday);
...
if (isset($_GET['dateActual'])) {
    ...
    $sql = 'SELECT a.id, a.username, a.ejsappid, a.practiceid, a.starttime, a.endtime, a.valid, b.name FROM 
{ejsappbooking_remlab_access} a INNER JOIN {ejsapp} b ON a.ejsappid = b.id WHERE a.username = "' . $USER->username .
        '" AND DATE_FORMAT(starttime, "%Y-%m-%d") >= "' . $today->format('Y-m-d') .
        '"  AND DATE_FORMAT(endtime, "%Y-%m") = "' . $date->format('Y-m') . '"ORDER BY a.starttime ASC';
